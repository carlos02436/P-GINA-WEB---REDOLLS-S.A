document.addEventListener('DOMContentLoaded', () => {
    ...
    addVacancyBtn.addEventListener('click', () => {
        ...
        newVacancyEntry.innerHTML = `
            <h5>Vacante ${vacancyCount}</h5>
            <div class="mb-3">
                <label for="vacancyTitle-${vacancyIdSuffix}" class="form-label">Título de la Vacante</label>
            ...
            <div class="mb-3">
                <label for="vacancyRequirements-${vacancyIdSuffix}" class="form-label">Requisitos</label>
    ...
    addServiceBtn.addEventListener('click', () => {
        serviceCount++;
        const serviceIdSuffix = `s${serviceCount}`; 
        const newServiceEntry = document.createElement('div');
        newServiceEntry.classList.add('service-entry', 'mb-3', 'p-3', 'border', 'rounded'); 
        newServiceEntry.setAttribute('id', `service-entry-${serviceIdSuffix}`);
        newServiceEntry.innerHTML = `
            <h5>Servicio ${serviceCount}</h5>
            <div class="mb-3">
                <label for="serviceName-${serviceIdSuffix}" class="form-label">Nombre del Servicio</label>
                <input type="text" class="form-control" id="serviceName-${serviceIdSuffix}" name="services[${serviceCount-1}][name]" required>
            </div>
            <div class="mb-3">
                <label for="serviceDescription-${serviceIdSuffix}" class="form-label">Descripción del Servicio</label>
                <textarea class="form-control" id="serviceDescription-${serviceIdSuffix}" name="services[${serviceCount-1}][description]" rows="3"></textarea>
            </div>
        `;
        const deleteBtn = createDeleteButton(() => {
            newServiceEntry.remove();
        });
        newServiceEntry.appendChild(deleteBtn);
        servicesContainer.appendChild(newServiceEntry);

        const firstInput = newServiceEntry.querySelector('input[type="text"]');
        if (firstInput) {
            firstInput.focus();
        }
    });
